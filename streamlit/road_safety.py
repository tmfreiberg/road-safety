This will not work since last update to .py files and models.
# Since model is useless anyway, the aim now is to have the user input features then find matching records in the dataset.
# The matches do not have to be exact: the user can specify how many feature matches they want, and how many features they definitely want to match.
# Then we would display the distribution of classes among the matches.
# We have functions that do this in retrospective.py.
...
from dictionaries import selection_dictionary, shorthand, selection_dictionary_shorthand, inverse_shorthand, \
    selection_dictionary_fr, shorthand_fr, selection_dictionary_shorthand_fr, inverse_shorthand_fr, FR_EN, explain
...
def english_content():
    ...
    with tab1:
        ...
        selected_options_df = pd.DataFrame(selected_options_dict, columns=selection_dictionary_shorthand.keys(),
                                           index=[0])
    ...
    with tab3:
        ...
        st.write(
            "QUEBEC AUTOMOBILE INSURANCE SOCIETY (SAAQ). Accident reports, [Dataset], in Data Quebec, 2017, updated December 18, 2023. [https://www.donneesquebec.ca/recherche/dataset/rapports-d-accident](https://www.donneesquebec.ca/recherche/dataset/rapports-d-accident) (accessed March 13, 2024).")
...
def french_content():
    ...
    with tab4:
        ...
        for idx, (feature_fr, options_fr) in enumerate(selection_dictionary_shorthand_fr.items()):
            ...
            selected_options_dict_fr[feature_fr] = selection_dictionary_fr[feature_fr][
                unencoded_selection_fr[feature_fr]]
            selected_options_dict = {FR_EN[feature_fr]: selected_options_dict_fr[feature_fr] for feature_fr in
                                     selected_options_dict_fr.keys()}
        # Create a DataFrame from selected options
        selected_options_df = pd.DataFrame(selected_options_dict, columns=selection_dictionary_shorthand.keys(),
                                           index=[0])
        ...
        # Display probabilities for all classes
        probabilities_placeholders_fr[0].text(
            f"Probabilité du niveau de gravité 0 : {100 * probabilities_fr[0, 0]:.2f}%")
        probabilities_placeholders_fr[1].text(
            f"Probabilité du niveau de gravité 1 : {100 * probabilities_fr[0, 1]:.2f}%")
        probabilities_placeholders_fr[2].text(
            f"Probabilité du niveau de gravité 2 : {100 * probabilities_fr[0, 2]:.2f}%")
    ...
    with tab5:
        ...
        if selected_key_fr == "GRAVITE":
            ...
            st.text(
                "Classe de gravité 0 : Dommages matériels inférieurs au seuil de rapportage/Dommages matériels seulement.")
    ...
    with tab6:
        ...
        st.write(
            "SOCIÉTÉ DE L'ASSURANCE AUTOMOBILE DU QUÉBEC (SAAQ). Rapports d'accident, [Jeu de données], dans Données Québec, 2017, mis à jour le 18 decembre 2023. [https://www.donneesquebec.ca/recherche/dataset/rapports-d-accident](https://www.donneesquebec.ca/recherche/dataset/rapports-d-accident).")
