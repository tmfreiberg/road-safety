...
# from saaq_documentation import FR_EN_map_default

# def combine_and_translate(
#     data_dir: Path = default_data_path, years: list = list(range(2011, 2023)), Print: bool = True
# ) -> dict:
#     csv = csv_filename(years=years)
#     if not files_exist(data_dir / csv[year] for year in years):
#         print(
#             "Ensure all of the above files are saved in the correct location, then try again."
#         )
#         return

#     print("\nLoading, combining and processing data from csv files".upper())
#     print("\nThis process may take up to around 30 seconds. There are five steps:\n")
#     # Initialize dictionary
#     rapport_annuel_d_accident = {}
#     # Keys are years 2011 - 2022, values are accident report dataframes.
#     print("1. Reading csv files into dataframes.")
#     for year in years:
#         rapport_annuel_d_accident[year] = pd.read_csv(data_dir / csv[year])
#     # Combine into one dataframe
#     print("2. Concatenating dataframes.")
#     rapports_d_accidents = pd.concat(
#         [rapport_annuel_d_accident[year] for year in years], axis=0
#     )
#     # Fix the names of three of the columns
#     print("3. Fixing column names.")
#     rapports_d_accidents.rename(
#         columns={
#             '\t\t\t"REG_ADM"': "REG_ADM",
...
#         inplace=True,
#     )
#     # The speed limit column consists of strings "<50", "50", etc.
#     # Convert the speeds to integers (except for "<50" and null values)
#     # .map(dictionary) does not work for some reason, so we apply the custom function above.
#     rapports_d_accidents["VITESSE_AUTOR"] = rapports_d_accidents["VITESSE_AUTOR"].apply(convert_to_integer)

#     # Make a copy that will be translated to English
#     print("4. Making a copy and translating column names to English.")
#     accident_reports = rapports_d_accidents.copy(deep=True)
#     # Translate most of the column names

#     accident_reports.rename(columns=FR_EN, inplace=True)
#     # Translate text in some columns
#     print("5. Translating text in certain columns to English.")


#     for FrColName in FR_EN_map_default.keys():
#         accident_reports[FR_EN[FrColName]] = accident_reports[FR_EN[FrColName]].map(
#             FR_EN_map_default[FrColName]
#         )
#     # Return a dictionary with English and French dataframes
#     print(
#         "\nProcess completed. Output is a dictionary whose two items are dataframes. Use key 'EN' for English, 'FR' for French."
#     )
#     output = {"EN": accident_reports, "FR": rapports_d_accidents}
#     return output

# def csv_filename(years: list, stem: str = "Rapport_Accident_") -> dict:
#     return {year: f"{stem}{year}.csv" for year in years}


# def files_exist(filenames: list) -> bool:
#     not_found = []
#     for filename in filenames:
#         if not filename.exists():
#             not_found.append(filename)
#     if not_found != []:
#         print("Can't find the files below.\n")
